...
export async function exportExcel({ filename, titre, sheets }) {
  ...
  for (const sheet of sheets) {
    ...
    sheet.rows.forEach((r) => ws.addRow(sheet.columns.map((c) => {
      const v = r[c.key];
      if ((sheet.dateKeys || []).includes(c.key) && v) {
        const d = new Date(v);
        return isNaN(d) ? v : d;
      }
      return v;
    ...
    // Dates affichées en jj/mm/aaaa (au lieu du format aaaa-mm-jj brut par défaut)
    (sheet.dateKeys || []).forEach((k) => {
      const idx = sheet.columns.findIndex((c) => c.key === k) + 1;
