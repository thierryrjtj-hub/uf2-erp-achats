...
import { CATEGORIES_MATIERES_PREMIERES } from "./categoriesArticles";
...
// garde que les catégories "matières premières" (voir categoriesArticles.js) ;
// si non fourni, aucun filtre par catégorie n'est appliqué.
export function articlesAReapprovisionner(frequences, seuilJoursAvance = 3, categorieParDesignation = null) {
  const aujourdHui = new Date();
  return frequences
    .filter((f) => f.cycleJours != null && f.nombreAchats >= 3 && f.derniereDate)
    .filter((f) => !categorieParDesignation || CATEGORIES_MATIERES_PREMIERES.includes(categorieParDesignation[f.designation]))
    .map((f) => {
