...
// Catégories qui ne suivent pas un vrai cycle de consommation régulier — elles
// s'achètent à la demande, sur planification ou par abonnement, pas parce
// qu'on en "manque" comme une matière première. Ne doivent jamais déclencher
// une alerte de réapprovisionnement.
const CATEGORIES_HORS_REAPPRO = ["Services & Prestations", "Fournitures de Bureau"];
...
// categorieParDesignation (optionnel) : { [designation]: categorie } — permet
// d'exclure les prestations et fournitures de bureau de l'alerte.
export function articlesAReapprovisionner(frequences, seuilJoursAvance = 3, categorieParDesignation = {}) {
  const aujourdHui = new Date();
  return frequences
    .filter((f) => f.cycleJours != null && f.nombreAchats >= 3 && f.derniereDate)
    .filter((f) => !CATEGORIES_HORS_REAPPRO.includes(categorieParDesignation[f.designation]))
    .map((f) => {
